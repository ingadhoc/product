...
class ProductPricelist(models.Model):
    ...
    def _price_rule_get_multi(self, pricelist, products_by_qty_by_partner):
        """ We send last_pricelist on the first call and then we add taxes
        only if this is the last pricelist (this is due to an error with
        pricelist recursive)

        NOTA: se hizo esto porque desde website_sale llama a
        price_rule_get_multi y no _price_get_multi, entonces modificamos
        _price_rule_get_multi
