...
class ProductPricelist(models.Model):
    ...
    def _get_products_price(self, products, quantity, date=False, uom_id=False, **kwargs):
        ...
        if self.env.context.get("taxes_included"):
            ...
            for product in products:
                ...
                if "pack_ok" in product._fields and self.check_for_product_pack_parent(product):
                    continue
                res[product.id] = product.taxes_id.filtered(lambda x: x.company_id.id == company_id).compute_all(
                    res[product.id], product=product
                )["total_included"]
        return res
    ...
    def check_for_product_pack_parent(self, product):
        ...
        if product.pack_ok and product.pack_component_price != "ignored":
            if product._name == "product.template" and product in self.item_ids.mapped("product_tmpl_id"):
                result = False
            elif product._name == "product.product" and (
                product in self.item_ids.mapped("product_id")
                or product.product_tmpl_id in self.item_ids.mapped("product_tmpl_id")
            ):
                result = False
            else:
                result = True
