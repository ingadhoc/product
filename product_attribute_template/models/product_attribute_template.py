...
class ProductAttributeTemplate(models.Model):
    ...
    name = fields.name = fields.Char(required=True)
    ...
    def _compute_products(self):
        for rec in self:
            ...
            rec.not_configured_product_qty = rec.product_tmpl_ids.search_count(
                [("attribute_line_ids.value_ids", "=", False), ("id", "in", rec.product_tmpl_ids.ids)]
            )

    def update_attributes(self):
        self.ensure_one()
        for product_tmpl in self.with_context(non_create_values=True).product_tmpl_ids:
            for attribute in self.product_attribute_ids - product_tmpl.attribute_line_ids.mapped("attribute_id"):
    ...
    def _compute_default_line_ids(self):
        ...
        res = [
            ...
            # create a new one
            if not product_template.attribute_line_ids.filtered(lambda x: x.attribute_id == attribute)
            # otherwise, return the line
            else (4, product_template.attribute_line_ids.filtered(lambda x: x.attribute_id == attribute)[0].id)
