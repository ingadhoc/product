...
class ReportReplenishmentBomStructure(models.AbstractModel):
    ...
    def _get_component_data(self, parent_bom, warehouse, bom_line, line_quantity, level, index, product_info, ignore_stock=False):
        ...
        price = bom_line.product_id.currency_id._convert(price, currency, (parent_bom.company_id or self.env.company), fields.Date.today())
    ...
    def _get_byproducts_lines(self, product, bom, bom_quantity, level, total, index):
        ...
        for byproduct in byproducts:
            ...
            price = byproduct_id.product_id.uom_id._compute_price(byproduct_id.product_id.standard_price, byproduct_id.product_uom_id) * line_quantity
    ...
    def _get_subcontracting_line(self, bom, seller, level, bom_quantity):
        ...
        currency = self.env.context.get('force_currency') or self.env.company.currency_id
        price = seller.currency_id._convert(seller.price, currency, (bom.company_id or self.env.company), fields.Date.today())
        res.update({
