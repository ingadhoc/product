...
class ProductReplenishmentCostRule(models.Model):
        ...
        for rec in self:
            description = "%s: %s" % (
                ...
                ", ".join(
                    rec.item_ids.mapped(
                        lambda x: "%s %s + %s%s"
                        % (x.name, x.percentage_amount, x.fixed_amount, "+ expr" if x.expr else "")
                    )
                ),
            )
    ...
    def _get_eval_context(self, obj=None):
        ...
        if not obj and self.env.context.get("active_id") and self.env.context.get("active_model") == "product.template":
    ...
    def compute_rule_inverse(self, cost):
        ...
        for line in self.item_ids.filtered("add_to_cost").sorted(reverse=True):
            cost = (cost - line.fixed_amount) / (1.0 + line.percentage_amount / 100.0)
        return cost
