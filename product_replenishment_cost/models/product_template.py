...
class ProductTemplate(models.Model):
    ...
    replenishment_cost_last_update = fields.Datetime(
        # sin tracking: el compute se dispara con cada cambio de regla y generaba un
        # mensaje de chatter por producto alcanzado. El dato queda en el campo.
    ...
    def cron_update_cost_from_replenishment_cost(self, limit=None, company_ids=None, batch_size=1000):
        ...
        for attempt in range(1, MAX_TRIES_ON_CONCURRENCY_FAILURE + 1):
            try:
                ...
                # cualquier otro error deja el job fallido: que se vea desde la interfaz
                # y no solo en el log del servidor.
                self._mark_running_run_as_error(exc, company_ids=company_ids)
                raise

    @api.model
    def _mark_running_run_as_error(self, exc, company_ids=None):
        """Deja registrado el fallo del cron, fuera de la transacción que se pierde.

        Primero se descarta la transacción del cron —está abortada o a punto de
        estarlo, y además hay que soltar los bloqueos que tenga sobre la corrida— y
        el registro se escribe en un cursor aparte, que sobrevive a la excepción que
        volvemos a lanzar. El rollback se lleva puesta la corrida si el fallo fue en
        el primer batch, todavía sin commit: en ese caso se crea de nuevo, ya en error.
        """
        self.env.cr.rollback()
        self.env.transaction.reset()
    ...
    def _cron_update_cost_from_replenishment_cost(self, limit=None, company_ids=None, batch_size=1000):
        """Actualiza el costo contable de a batches, encadenando el cron hasta terminar.

        El estado de la pasada (cursor, contadores, detalle de lo que cambió) vive en
        ``product.replenishment_cost.run``: todos los batches encadenados comparten la
        misma corrida, así "cuándo terminó y qué cambió" se puede responder desde la
        interfaz. El commit por batch se mantiene: sin él, un batch largo puede chocar
        con otros procesos y perder todo el trabajo hecho.
        ...
        # traemos un registro de más para saber si quedan pendientes sin recorrer la tabla entera
        ...
        # si quedan pendientes, volvemos a llamar al cron para el próximo batch
        if pending:
    ...
    def _update_cost_from_replenishment_cost(self, run=None):
        ...
        Si recibimos una corrida (``run``) registramos ahí los contadores y una línea
        por producto que efectivamente cambió de costo, con el valor anterior y el nuevo.
