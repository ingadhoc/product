...
class ProductTemplate(models.Model):
        ...
        for attempt in range(1, MAX_TRIES_ON_CONCURRENCY_FAILURE + 1):
            try:
                ...
                if attempt == MAX_TRIES_ON_CONCURRENCY_FAILURE:
                    ...
                    self._mark_running_run_as_error(exc)
                    raise
    ...
    @api.model
    def _mark_running_run_as_error(self, exc):
        """Cierra como error la corrida en curso, en su propia transacción.

        Se llama con la transacción abortada o a punto de abortarse, así que primero
        se limpia el estado y recién ahí se escribe.
        """
        self.env.cr.rollback()
        self.env.transaction.reset()
        run = (
            self.env["product.replenishment_cost.run"]
            .sudo()
            .search([("state", "=", "running"), ("trigger", "=", "cron")], limit=1)
        )
        if not run:
            return False
        run._finish(state="error", error_message="%s: %s" % (exc.__class__.__name__, exc))
