...
class ProductReplenishmentCostRun(models.Model):
    ...
    @api.model
    def _log_cron_failure(self, error_message, company_ids=None):
        """Deja el fallo del cron a la vista, no solo en el log del servidor.

        Cierra como error la corrida en curso. Si el fallo se llevó puesta la corrida
        —falló el primer batch, cuando todavía no hubo commit— se crea una corrida en
        error igual: si no, un cron que falla de entrada no deja rastro en ningún lado.
        No se usa ``_create_run`` para no consumir el cursor viejo de una pasada que
        ...
        run._finish(state="error", error_message=error_message)
        return run
