    ...
    def _compute_discount_restriction(self):
        ...
        for wizard in self:
            if (
                ...
                and wizard.sale_order_id.pricelist_id
                in user.discount_restriction_ids.mapped('pricelist_id')
            ):
    ...
    def _check_discount_type(self):
        if self.filtered('discount_restriction'):
            raise ValidationError(
                _("You do not have permission to apply this type of discount "
                "with the current pricelist.")
            )
