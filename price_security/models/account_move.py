    ...
    def _get_view(self, view_id=None, view_type="form", **options):
        ...
        if view_type == "form":
            if self.env.user.has_group("price_security.group_only_view"):
                fields = arch.xpath("//field[@name='invoice_line_ids']/list//field[@name='price_unit']") + arch.xpath(
                    "//field[@name='invoice_line_ids']/list//field[@name='tax_ids']"
                )
                for node in fields:
                    node.set(
                        "readonly",
                        "parent.move_type in ('out_invoice', 'out_refund') and product_can_modify_prices == False",
                    )
