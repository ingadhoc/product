...
class ProductTemplate(models.Model):
    ...
    def _compute_replenishment_cost(self):
        ...
        for rec in bom_costs:
            ...
            price = 0.0
            bom = self.env['mrp.bom']._bom_find(rec.product_variant_ids[0])[rec.product_variant_ids[0]] if self.env['mrp.bom']._bom_find(rec.product_variant_ids[0])[rec.product_variant_ids[0]] else self.env['mrp.bom']._bom_find(rec.with_context(active_test=False).product_variant_ids[0])[rec.with_context(active_test=False).product_variant_ids[0]]
            if not bom:
                rec.update({
