...
class ProductPricesUpdateWizard(models.TransientModel):
    ...
    price_type = fields.Selection([("list_price", "Sale Price"), ("standard_price", "Cost Price")], required=True)
        ...
        if self.check is True:
            ...
            for prodct in self.env["product.template"].browse(active_ids):
                if self.price_type == "list_price":
                    ...
                    raise UserError(_('Price type "%s" is not ' "implemented") % (self.price_type))
                new_price = self.calc_new_price(old_price, self.price_discount, self.price_surcharge, self.price_round)
                vals = {
                    "product_tmpl": prodct,
                    "new_price": new_price,
                }
    ...
    @api.model
    def calc_new_price(self, old_price, price_discount, price_surcharge, price_round):
        new_price = old_price * (1.0 + (price_discount or 0.0))
        if price_round:
            new_price = tools.float_round(new_price, precision_rounding=price_round)
        if price_surcharge:
...
class ProductPricesUpdateWizardResultDetail(models.TransientModel):
    ...
    result_id = fields.Many2one("product.prices_update_wizard_result", "Result")
...
class ProductPricesUpdateWizardResult(models.TransientModel):
    ...
    def _get_details(self):
        ...
        for product_tmpl in self.env["product.template"].browse(product_tmpl_ids):
            ...
            vals = {
                ...
                "old_price": old_price,
                "new_price": self.env["product.prices_update_wizard"].calc_new_price(
                    old_price, price_discount, price_surcharge, price_round
                ),
            }
            ret.append(vals)
        return ret
    ...
    def confirm(self):
        ...
        for line in self.detail_ids:
            ...
            products_vals.append(vals)
        return self.env["product.prices_update_wizard"].update_prices(products_vals, price_type)
